...
const MyToys = () => {
    ...
    useEffect(() => {
        fetch(`${import.meta.env.VITE_BASE_URL}/my-toys?email=${user?.email}`).then(data => data.json()).then(data => setToys(data))
    }, [count])
    ...
    const sortdata = (data) => {
        ...
        fetch(`${import.meta.env.VITE_BASE_URL}/my-toys/sort?email=${user?.email}&sorting=${data}`).then(data => data.json()).then(data => setToys(data))
    ...
    const deletedata = (data) => {
        swal({
            ...
            .then((Delete) => {
                if (Delete) {
                    fetch(`${import.meta.env.VITE_BASE_URL}/my-toys`, {
                        method: "DELETE",
    ...
    return (
        <div className='min-h-[100vh]'>
          <div className="flex flex-col sm:flex-row justify-center my-8 gap-3 items-center">
                <Link to="/" className="button">Home</Link>
                <Link to="/all-toys" className="button">All Toys</Link>
                <div className='bg-blue-900 rounded-md text-white px-4 py-2'>
                    {toys.length > 0 ? (
                        <div className="space-y-2">
                            <p className='font-bold'>Name: {toys[0].sellerName}</p>
                            <p className='font-bold'>Email: {toys[0].sellerEmail}</p>
                        </div>
                    ) : (
                        <p className='text-bold'>No seller data</p>
                    )}
                </div>
...

                        {
                            toys.map(data =>
                                <tr key={data._id} className="hover ">
                                    ...
                                    <td>{data.category}</td>
                                    <td className="pl-[20px]">{data.price}</td>
                                    <td className="pl-[20px]">{data.quantity}</td>
                                    <td className="pl-[20px]">{data.rating}</td>
                                    <td>
